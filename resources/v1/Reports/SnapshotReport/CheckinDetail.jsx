...
import {  CHECKIN_CHECKOUT_DATA,CHECKIN_CHECKOUT_EXCEL_DATA ,RIDER_SNAPSHOT_DATA_DATEWISE,DOWNLOAD_SNAPSHOT_DATA_DATEWISE} from "../../Auth/Context/AppConstant";
...
const CheckinDeail = (props) => {
    ...
    let search_value = props.data.search ? props.data.search : "";
    ...
    const [q, setQ] = useState(search_value);
  ...
  function closeModal() {
    ...
    const getPage = (activePage) => {
            ...
            axios.get( RIDER_SNAPSHOT_DATA_DATEWISE + '?q=' + q + '&page=' + activePage+'&checkedin_status=' + selectedCheckinStatus + '&client_id=' + selectedClient + '&hub_id=' + selectedHub+ '&state=' + selectedState+ '&city=' + selectedCity+"&start_date="+startDate+"&end_date="+endDate+"&selectedDate="+selectedDate+"&selectedTab="+selectedTab)
            .then(res => {
    ...
    const searchValue = () => {
        ...
        axios.get(CHECKIN_CHECKOUT_DATA  + '?q=' + q + '&page=' + 1+'&checkedin_status=' + selectedCheckinStatus + '&client_id=' + selectedClient + '&hub_id=' + selectedHub+ '&state=' + selectedState+ '&city=' + selectedCity+"&start_date="+startDate+"&end_date="+endDate+"&selectedDate="+selectedDate+"&selectedTab="+selectedTab)
            .then(res => {
    ...
    const getCheckinList = async () => {
        ...
        await axios.get(RIDER_SNAPSHOT_DATA_DATEWISE + '?q=' + q + '&page=' + activePage+'&checkedin_status=' + selectedCheckinStatus + '&client_id=' + selectedClient + '&hub_id=' + selectedHub+ '&state=' + selectedState+ '&city=' + selectedCity+"&start_date="+startDate+"&end_date="+endDate+"&selectedDate="+selectedDate+"&selectedTab="+selectedTab)
            .then(res => {
                if (res.data.success === true) {
                    setDataAndPagination(res.data.data);
                }
   ...
   const setExcelData = (checkinOutData,moduleData) => {
       ...
       if(excelDownloadData.length > 0 ){
        ...
        excelDownloadData.map((attendance, i) => (
            ...
            custs.push({
                SlNo:  i < excelDownloadData.length ? i+1:'',
                ...
                Checkin_Status :attendance ? attendance.attendance_status :'',
                Checkin_App : attendance ?  helpers.checkinMode(attendance.vehicle_type) : '',
                Checkin_Time: attendance && attendance.ll_logintime  != null &&  attendance.ll_logintime  !="No Checkin Done" ?  moment(attendance.ll_logintime).format('HH:mm:ss'):'',
                Checkout_Time : attendance && attendance.ll_logouttime  != null &&  attendance.ll_logouttime  !='No Checkout Done' ?  moment(attendance.ll_logouttime).format('HH:mm:ss') :'',
                Total_Time :  attendance && attendance.ll_logouttime  != null  &&  attendance.ll_logouttime  !='No Checkout Done' ?   helpers.dateDifference(attendance.ll_logintime,attendance.ll_logouttime ):'' ,
    ...
    const exportToCSVData = (moduleData)=>{
        ...
        let todayDate = moment(currentDateTime).format("YYYY-MM-DD");
       ...
       if(selected_date == todayDate){
           ...
           axios.get(CHECKIN_CHECKOUT_EXCEL_DATA  + '?q=' + search_value + '&page=' + 1+'&checkedin_status=' + selectedCheckinStatus + '&client_id=' + cliend_id + '&hub_id=' + hub_id+ '&state=' + state+ '&city=' + city+"&start_date="+startDate+"&end_date="+endDate+"&selectedDate="+selected_date)
           ...
           axios.get(DOWNLOAD_SNAPSHOT_DATA_DATEWISE  + '?q=' + q + '&page=' + 1+'&checkedin_status=' + selectedCheckinStatus + '&client_id=' + cliend_id + '&hub_id=' + hub_id+ '&state=' + state+ '&city=' + city+"&start_date="+startDate+"&end_date="+endDate+"&selectedDate="+selected_date)
                                        ...
                                        <div className="card-body table-card">
                                            <div className="row">
                                                <div className="col-md-6">
                                                    <h4 className="mt-0 header-title  pl-3">Attendance Report For {title}</h4>
                                                </div>
                                                    ...
                                                    <tbody>
                                                        {
                                                            checkinOutData.length  > 0 ? checkinOutData.map((attendance, i) => (
                                                                <tr key={i}>
                                                                    <td className="sticky-col first-col">  {attendance  ?  checkvaliddate(attendance.date) : ''} </td>
                                                                    ...
                                                                    <td> {attendance ? attendance.attendance_status : ''}</td>
                                                                    
                                                                    <td> {attendance ?  helpers.checkinMode(attendance.vehicle_type) : ''}</td>
                                                                    <td> {
                                                            
                                                                        attendance && attendance.emp_id  && attendance.ll_logintime != "No Checkin Done"  && attendance.ll_logintime !="" && attendance.ll_logintime  != null ? moment(attendance.ll_logintime).format('HH:mm:ss') : ''
                                                                        
                                                                    
                                                                    } 
                                                                    </td>
                                                                    <td> {
                                                                    attendance && attendance.emp_id  && attendance.ll_logouttime  != "No Checkout Done" &&  attendance.ll_logouttime  != ""  && attendance.ll_logouttime  != null?  moment(attendance.ll_logouttime).format('HH:mm:ss') : ''} </td>
                                                                    <td> 
                                                                     {
                                                                        attendance  && attendance.emp_id && attendance.ll_logouttime  != null && attendance.ll_logouttime  != "No Checkout Done"  ?  helpers.dateDifference(attendance.ll_logintime,attendance.ll_logouttime ) :''
                                                            ...
                                                            <td> {attendance ? attendance.remarks : ''}</td>
                                                                ...
                                                                <TableNoDataFound message={'No Chckin Data Found!'} frontSpan={3} backSpan={2} />
                                                        }
                                                    </tbody>
                                                </table>
                                            </div>
