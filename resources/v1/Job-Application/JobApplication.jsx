...
//job?.jobs?.cities?.[0]?.city?.cityName
const JobApplication = () => {
  ...
  function searchHandler(e) {
    ...
    const firstName =
      typeof e === "string"
        ? e?.toLowerCase()
        : e[0]?.leadData?.llempFirstname?.toLowerCase();
    const contactNumber =
      typeof e === "string"
        ? e?.toLowerCase()
        : e[0]?.leadData?.llempContactNumber?.toLowerCase();
    const id =
      typeof e === "string"
        ? e?.toLowerCase()
        : e[0]?.id?.toString()?.toLowerCase();
    if (e.length > 0) {
      tempData = tempData.filter(
        (data) =>
          data?.leadData?.llempFirstname.toLowerCase().includes(firstName) &&
          data?.leadData?.llempContactNumber
            .toLowerCase()
            .includes(contactNumber) &&
    ...
    // tempData = tempData.filter(
    //   (data) =>
    //     data?.leadData?.llempFirstname
    //       .toLowerCase()
    //       .includes(e[0].leadData?.llempFirstname?.toLowerCase()) &&
    //     data?.leadData?.llempContactNumber
    //       .toLowerCase()
    //       .includes(e[0].leadData?.llempContactNumber?.toLowerCase()) &&
    //     data?.id
    //       ?.toString()
    //       ?.toLowerCase()
    //       .includes(e[0].id?.toString()?.toLowerCase())
    // );
                ...
                <div className="col-sm-4">
                  <div className="search-input">
                    <Typeahead
                      ...
                      // labelKey={(option) =>
                      //   `${option?.id} - ${option?.leadData?.llempFirstname} - ${option?.leadData?.llempContactNumber}`
                      // }
                      ref={ref}
                      labelKey={(option) => getOptions(option)}
                      options={jobDetails}
                      onChange={(e) => {
                                  ...
                                  <tr>
                                    ...
                                    <td>
                                      ...
                                      {job?.applicationStatus
                                        ? userStatusMap[job?.applicationStatus]
                                        : "-"}
                                    </td>
                                    <td>
                                      {job?.appliedOn
