...
//import ChargingContext from '../../Charging/Context/ChargingContext';
import RbacContext from "../Context/RbacContext";

//import { moduleData } from "../Context/RbacContext";
...
const List = (props) => {
    ...
    const breadCrumbs = [
        ...
        { name: "UploadDatas", url: "#", class: "breadcrumb-item active" }
    ];
   ...
   const setExcelData = (data,moduleData) => {
    ...
    const exportToCSVData = (moduleData)=>{
        ...
        search_value = textInput.current.value;
        axios.get(DOWNLOAD_XL_DATA + '?q=' + textInput.current.value + '&page=' + 1)
            .then(res => {
                if (res.data.success === true) {
                        ...
                        <div className="container-fluid">
                            <div className="page-title-box">
                                <div className="row align-items-center">
                                    <Title title={'UploadData List'} />
                                </div>
                                <BreadCrumb breadCrumbs={breadCrumbs} />
                                <div className="row align-items-center">
                                    <div className="col-sm-3">
                                        <input ref={textInput} type="text" className="form-control" placeholder="Month/ClientEMPID" />
                                    </div>
                                        ...
                                        <div className="card-body table-card">
                                            <div className="row">
                                                <div className="col-md-6">
                                                    <h4 className="mt-0 header-title  pl-3">Uploaded Datas</h4>
                                                </div>
                                                <div className="col-md-6 text-right ">
                                                    <Link to="/app/access-app/fileupload/add"
                                                        className="btn btn-primary btn-sm waves-effect waves-light mr-3">
                                                        <i className="fa fa-plus mr-1"> </i>Upload New</Link>


                                                        <div className="btn btn-primary waves-effect waves-light mr-3 exceldownload-btn"
                                                            ...
                                                            paymentData.length > 0 ? paymentData.map((payment, i) => (
                                                                ...
                                                                <tr key={i}>
                                                                   ...
                                                                   <td className={'word-break'}> 
                                                                     <Link
                                                                            to={'/app/access-app/fileupload/edit/' + payment._id}>{payment.empleads.llemp_firstname}{payment.empleads.llemp_lastname}
                                                                        </Link> </td>
                                                                    ...
                                                                    <td className={'word-break'}> {payment.client}</td>
                                                                    <td className={'word-break'}> {payment.hub_location}</td>
                                                                    <td className={'word-break'}> {payment.city}</td>
                                                                    <td className={'word-break'}> {payment.state}</td>
                                                                    <td className={'word-break'}> {payment.full_time}</td>
                                                                    <td className={'word-break'}> {payment.vendor}</td>
                                                                    <td className={'word-break'}> {payment.vendor_type}</td>
                                                                    <td className={'word-break'}> {payment.vehicle_type}</td>
                                                                    <td className={'word-break'}> {payment.vehicle_model}</td>
                                                                    ...
                                                                    <Link
                                                                        to={'/app/access-app/fileupload/edit/' + payment._id }>
